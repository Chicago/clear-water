...
col_list = ['1_day_prior_Ecoli_geomean','2_day_prior_Ecoli_geomean','3_day_prior_Ecoli_geomean',
            '4_day_prior_Ecoli_geomean','5_day_prior_Ecoli_geomean',
            '6_day_prior_Ecoli_geomean','7_day_prior_Ecoli_geomean']
temp = dfn[col_list].copy()
dfn['7_day_moving_avg'] = temp.mean(axis=1)


col_list = ['Timestamp','Beach','Month','Weekday','Ecoli_geomean','1_day_prior_Ecoli_geomean','7_day_moving_avg']
df = dfn[col_list].copy()
df['PriorReading'] =  pd.Series(map((  lambda x,y: y if isnan(x) else x  ), df['1_day_prior_Ecoli_geomean'], df['7_day_moving_avg'])  )  
df.drop(['1_day_prior_Ecoli_geomean'], axis=1, inplace=True)
