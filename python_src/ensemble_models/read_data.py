...
def read_data(verbose=False, read_drek=True, read_holiday=True, read_weather_station=True,
              ...
              group_beaches=True, add_each_beach_data=False):
    '''
    ...
    # after cleaning. Create some features based on daily forcast
    if read_daily_forecast:
        # Interaction variables combining wind speed and direction
        ...
        # Cumulative rain variable to try to capture storm events
        df = add_column_prior_data(df, 'precipIntensityMax', [1,2,3,4] , beach_col_name='Client.ID', timestamp_col_name='Full_date')
    ...
    df['BEACH'] = df['Client.ID']
    df['Client.ID'] = df['Client.ID'].map(lambda x: beach_dict[x])
