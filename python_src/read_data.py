...
def read_data_simplified():
    ...
    for yr in range(2002,2016):
        if yr != 2004:
            if yr == 2015:
                file_name = cpd_data_path + str(yr) + ' Lab Results.xlsx'
            else:
    ...
    df.insert(0, 'DayofYear', df['Timestamp'].dt.dayofyear)
    df.drop(['Date'], axis=1,inplace=True )

    df['Beach'] = df['Beach'].map(lambda x: x.strip())

    df = df[['Full_date','Timestamp','Beach','Year','DayofYear',
             'Weekday','lowReading','highReading','Ecoli_geomean']]

    # clean up beaches and delete weekend data readings
    df = df.ix[(df['Weekday']!='Saturday')&(df['Weekday']!='Sunday')]
    df = df.reset_index()
    df.drop(['index'], 1, inplace=True)
    dfn = clean_up_beaches(df)
    dfn.drop(['Beach-Date'],1,inplace=True)
...
def group_beaches_geographically(data, beach_names_column='Client.ID', verbose=False):
    ...
    # Mid North
    group_1 = ['Montrose','Montrose Dog','Foster','Osterman','Thorndale']

    # Rogers Park Area
    group_2 = ['Juneway','Rogers','Howard','Marion','Leone','Pratt','Columbia','North Shore','Hartigan','Albion',
            'Marion Mahoney Griffin','Loyola','Margaret T Burroughs','Jarvis','Foster']
    # Near North Area
...
def add_column_prior_data(df, colnames, ns, beach_col_name='Beach', timestamp_col_name='Timestamp'):
    ...
    for colname in colnames:
        for n in ns:
            ...
            #days = list(pd.Series(df[timestamp_col_name].unique())) # this would take previous reading rather than previous day
            ...
            final = final.dropna(subset=[colname]) # without this there is a row for *every* day 
...
def add_columns_each_beach(df, beach_col_name='Beach', timestamp_col_name='Timestamp', ecoli_col_name='Ecoli_geomean'):
    ...
    '''
    dfc = df.copy()
    dfc = add_column_prior_data(dfc, ecoli_col_name, [1,2,3,4,5,6,7] , beach_col_name , timestamp_col_name)
...
def process_hourly_data(df, hours_offset=0, beach_col_name='Client.ID', timestamp_col_name='Full_date'):
    '''
...
def read_data(verbose=False, dropWeekend=False, read_drek=False, read_holiday=False, read_weather_station=False, read_water_sensor=False, read_daily_forecast=False, read_hourly_forecast=False, group_beaches=False):
    '''
    ...
    # Final cleaning, drop rows with no beach name, remove duplicate beach-days, remove entries with no Ecoli reading
